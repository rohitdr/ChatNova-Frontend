...
export default function Group() {
  ...
  const handleChange = ({ target: { value } }) => {
    if (value.trim().length === 0) {
      ...
      let filteredgroups =
        allGroup?.filter((grp) =>
          grp.name.toString().includes(value.toString())
        ) || [];
  ...
  const allNormalizedGroups = useMemo(() => {
    return allGroup?.map((element) =>
      normalizeItem(element, "chat")
    );
  }, [normalizeItem, allGroup]);

  const filteredNormalizedGroups = useMemo(() => {
    return filteredGroups?.map((element) =>
    ...
    <>
      <div className="h-screen 2xs:p-0 xs:p-1 lg:p-0 flex bg-gradient-to-br from-indigo-50 to-purple-50 flex-col">
        ...
        <div className="flex justify-between">
          ...
          <div className="pt-2">
            <PlusIcon
              className="w-6 h-6 mx-2 text-blue-700 cursor-pointer"
          ...
          {!searchClick &&
            ...
            filteredNormalizedGroups.map((element) => (
              <UserItem
                key={element._id}
                user={element}
                handleUserClick={handleGroupClick}
              />
            ))}
