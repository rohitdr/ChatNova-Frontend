...
import {  ArrowUpCircleIcon, EllipsisVerticalIcon,PencilIcon,UserCircleIcon  } from "@heroicons/react/24/solid";
import { useContext, useState } from "react";
import AuthContext from "../Context/AuthContext";
import { PencilSquareIcon } from "@heroicons/react/24/outline";
export default function Settings() {
  const authContext = useContext(AuthContext)
  const {user,updateUserImage} = authContext
...
/>
           {settingsImage?<ArrowUpCircleIcon className="w-7 h-7 right-2 bg-white shadow text-blue-900    cursor-pointer rounded-full bottom-3 absolute " onClick={()=>{ updateUserImage(settingsImage);setSettingsImage(null)}}></ArrowUpCircleIcon> :<PencilIcon className="w-7 h-7 right-2 bg-white border border-black  p-1.5 text-blue-900  cursor-pointer rounded-full bottom-3 absolute " onClick={()=>{document.getElementById("settingsImage").click()}}></PencilIcon>}
        <img className='w-28  shadow h-28 rounded-full   border-2' src={settingsImage?URL.createObjectURL(settingsImage):user?.image.url} alt="" />
