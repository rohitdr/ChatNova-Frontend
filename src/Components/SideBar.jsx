...
import { UserCircleIcon,ChatBubbleLeftRightIcon,Cog6ToothIcon,UserGroupIcon,PowerIcon, AdjustmentsVerticalIcon, Cog8ToothIcon  } from "@heroicons/react/24/solid";
import AuthContext from '../Context/AuthContext';

export default function SideBar() {
    const  context = useContext(AuthContext)
    const {logout,user,setActivePage,activePage}=context
 ...
 logout()
  ...
  return (
   ...
   <div className='flex lg:h-[20rem] h-full flex-row lg:flex-col items-center justify-between'>
    <div className=''>    <UserCircleIcon onClick={()=>{setActivePage(1)}} className={`cursor-pointer md:w-9 w-7 h-7 sm:ml-12 lg:w-9 lg:h-9 lg:mb-4 mx-4 lg:mx-2 md:h-9 text-black ${activePage === 1 ?"text-blue-700":"text-black"}`} /></div>
    <div> <ChatBubbleLeftRightIcon onClick={()=>{setActivePage(0)}} className={`lg:w-9 sm:ml-12 w-7 h-7 cursor-pointer md:w-9 md:h-9 2xs:mx-2 xs:mx-4 lg:my-2 lg:mx-2 lg:h-9  ${activePage === 0 ?"text-blue-700":"text-black"}`} /></div>
    <div > <UserGroupIcon onClick={()=>{ setActivePage(2) }} className={`lg:w-9  w-7 md:w-9 lg:my-2 h-7 md:h-9 sm:ml-12 cursor-pointer lg:h-9 mx-4 lg:mx-2  ${activePage === 2 ?"text-blue-700":"text-black"}`} /></div>
    <div> <Cog8ToothIcon onClick={()=>{setActivePage(3)}} className={` cursor-pointerlg:w-9 md:w-9 w-7 sm:ml-12 lg:my-6 h-7 md:h-9 2xs:mx-2 2xs:mt-0 cursor-pointer xs:mx-4 lg:mx-2 lg:h-9  ${activePage === 3 ?"text-blue-700":"text-black"}`} /></div>
   ...
   <img onClick={()=>{profileOptionsActive?setProfileOptionsActive(false):setProfileOptionsActive(true)}} src={user?.image.url} alt="" className=' 2xs:h-7 2xs:w-7 border-black cursor-pointer border-2 2xs:mr-2 xs:mr-5 lg:mx-0 lg:h-10 lg:w-10 h-8 w-8 xs:mb-1 rounded-full' />
   </div>
