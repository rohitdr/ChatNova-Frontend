...
export default function ForgetPassword() {
  ...
  const { forgetPassword, isServerDown,showAlert } =  useContext(AuthContext);
...
const isFormValid=emailRegex.test(formData.email.trim()) && formData.password.length>=8 && formData.username.trim().length>=8
  ...
  await forgetPassword(formData.email.trim(),formData.password,formData.username.trim())
    setFormData({
    username: "",
    password: "",
    email: "",
  ...
  <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-100 to-purple-100  px-4">
    ...
    <div className="w-full max-w-md bg-white/80 backdrop-blur-xl shadow-2xl rounded-2xl p-8">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div>
          <label className="text-sm text-gray-600" htmlFor="forget-email">Email</label>
        ...
        <div>
          <label className="text-sm text-gray-600"  htmlFor="forget-username">Username</label>
          <div className="flex items-center mt-1 rounded-lg border border-gray-300 focus-within:ring-2 focus-within:ring-indigo-400">
        ...
        <div>
          <label className="text-sm text-gray-600" htmlFor="forget-password">New Password</label>
          <div className="flex items-center mt-1 rounded-lg border border-gray-300 focus-within:ring-2 focus-within:ring-indigo-400">
